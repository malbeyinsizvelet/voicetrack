...
export const countCompletedLines = (task: Task): number => (task.lines ?? []).filter((l) => l.status === 'approved').length;
export const countRecordedLines  = (task: Task): number => (task.lines ?? []).filter((l) => l.status === 'recorded').length;
export const countPendingLines   = (task: Task): number => (task.lines ?? []).filter((l) => l.status === 'pending').length;
export const countRejectedLines  = (task: Task): number => (task.lines ?? []).filter((l) => l.status === 'rejected' || l.status === 'retake').length;

export function formatLineCount(completed: number, total: number): string { return `${completed} / ${total}`; }

export function progressToColorClass(_percent: number): { ring: string; bar: string; text: string } {
  return { ring: 'stroke-current', bar: 'vt-progress-fill', text: 'vt-text-primary' };
}
...
export function computeCastProgress(_project: Project, task: Task): CastProgress {
  ...
  return { taskId: task.id, characterId: task.characterId ?? '', characterName: task.characterName, assignedArtistName: task.assignedArtistName ?? 'Atanmamış', totalLines, completedLines, approvedLines, recordedLines, pendingLines, rejectedLines, retakeLines, progressPercent, taskStatus: task.status, priority: 'normal' };
}
...
export function computeProjectProgress(project: Project): ProjectProgress {
  ...
  const progressPercent = pct(completedLines, totalLines);
  const tasksByStatus = tasks.reduce<Record<string, number>>((acc, t) => { acc[t.status] = (acc[t.status] ?? 0) + 1; return acc; }, {});
  const castProgresses: CastProgress[] = tasks.map((t) => computeCastProgress(project, t));

  return { projectId: project.id, projectTitle: project.title, totalCasts, completedTasks, inProgressTasks, uploadedTasks, totalLines, completedLines, recordedLines, pendingLines, rejectedLines, progressPercent, tasksByStatus, castProgresses };
}
...
export function computeOverallProgress(projects: Project[]) {
  ...
  return { totalProjects, activeProjects, totalLines, completedLines, recordedLines, pendingLines, overallPercent, projectProgresses: progresses };
}

...
function ToastItem({ toast, onDismiss }: { toast: Toast; onDismiss: () => void }) {
  ...
  return (
    <div
      className={cn(
        'relative flex items-start gap-3 px-4 py-3.5 rounded-xl border shadow-xl',
        'w-80 overflow-hidden animate-toast-in',
        cfg.classes
      )}
      role="alert"
    >
      ...
      <div
        ...
        style={{
          width: '100%',
          animation: `progressFill ${toast.duration ?? 4000}ms linear forwards`,
        }}
      />

      {cfg.icon}
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-slate-100 leading-snug">{toast.title}</p>
        {toast.message && (

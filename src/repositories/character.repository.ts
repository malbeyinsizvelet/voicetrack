...
// Karakter ve task CRUD — proje store üzerinde çalışır.
// Gerçek sistemde: /api/projects/:id/characters endpoint'leri
// ============================================================
...
export const characterRepository = {
  ...
  async create(
    ...
    const task: Task = {
      ...
      assignedTo: data.assignedArtistId ?? '',
      assignedArtistName: data.assignedArtistName ?? 'Atanmamış',
      lineCount: data.lineCount ?? 0,
      status: 'pending',
      sourceFiles: [],
      recordedFiles: [],
  ...
  async update(
    ...
    if (charIdx === -1) {
      log.warn('update — character not found', { projectId, characterId });
      return null;
    }
    ...
    projectRepository._updateStore(projectId, (p) => {
      // Task sync: lineCount veya assignee değiştiyse task'ı güncelle
      const tasks = p.tasks.map((t) => {
        if (t.id !== oldChar.taskId) return t;
        return {
          ...t,
          ...(data.lineCount !== undefined ? { lineCount: data.lineCount } : {}),
          ...(data.assignedArtistId !== undefined ? { assignedTo: data.assignedArtistId } : {}),
          ...(data.assignedArtistName !== undefined ? { assignedArtistName: data.assignedArtistName } : {}),
          updatedAt: ts,
        };
      });
  ...
  async assignArtist(
    ...
    return this.update(projectId, characterId, {
      assignedArtistId: payload.artistId,
